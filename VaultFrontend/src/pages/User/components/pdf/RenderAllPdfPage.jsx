...
import { base64ToArrayBuffer } from "../../constant/Utils.jsx";
...
function RenderAllPdfPage(props) {
  ...
  return (
    <div ref={pageContainer} className="w-[20%] bg-base-100">
      ...
      >
        <Document
          ...
          file={pdfDataBase64}
        >
          {Array.from(new Array(props?.allPages?.totalPages || 0), (el, index) => (
            <div
              key={index}
              className={`${
                props?.pageNumber - 1 === index
                  ? "border-[red]"
                  : "border-[#878787]"
              } border-2 m-[10px] flex justify-center items-center relative cursor-pointer hover:border-blue-500 transition-colors`}
              onClick={() => {
