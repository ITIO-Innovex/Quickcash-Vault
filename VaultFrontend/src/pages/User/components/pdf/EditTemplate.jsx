...
const EditTemplate = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData?.Note?.length > maxNoteLength) {
      alert(("note-length-alert"));
      return;
    }
    if (formData?.Description?.length > maxDescriptionLength) {
      alert(("description-length-alert"));
      return;
    }
    let pdfUrl;
    if (uploadPdf?.base64) {
    ...
    if (AutoReminder && reminderCount > 15) {
      alert(("only-15-reminder-allowed"));
      return;
    }
    if (AutoReminder) {
              ...
              <label className="block">
                ...
                <Tooltip id="istourenabled-tooltip" className="z-50">
                  <div className="max-w-[200px] md:max-w-[450px]">
                    <p className="font-bold">{("enable-tour")}</p>
                    <div className="p-[5px]">
                      <ol className="list-disc">
                        <li>
                          <span className="font-bold">{("yes")}: </span>
                          <span>{("istourenabled-help.p1")}</span>
                        </li>
                        <li>
                          <span className="font-bold">{("no")}: </span>
                          <span>{("istourenabled-help.p2")}</span>
                        </li>
                      </ol>
                    </div>
                    <p>{("istourenabled-help.p3", { appName: appName })}</p>
                  </div>
                </Tooltip>
              </label>
              <div className="flex flex-col md:flex-row md:gap-4">
              ...
              <label>
                ...
                <Tooltip id="nos-tooltip" className="z-[999]">
                  <div className="max-w-[200px] md:max-w-[450px] text-[11px]">
                    <p className="font-bold">{("notify-on-signatures")}</p>
                    <p>{("notify-on-signatures-help.p1")}</p>
                    <p>{("notify-on-signatures-help.note")}</p>
                  </div>
                </Tooltip>
              </label>
              <div className="flex flex-col md:flex-row md:gap-4">
